		...
		var handler = {
			'object': function createMultiConsumerHandler(handlers) {
				return function (data, ack, nack, msg) {
					if (typeof handlers[msg.fields.routingKey] === 'function') {
						return 	handlers[msg.fields.routingKey](data)
						.then(ack)
						.catch(nack)
						.error(nack);
					} else {
						// ideally this should forward to the deadletter queue/exchange
						return nack('No handler for given routingKey')
					}
				};
			},
